...
public class Object extends Entity {
	...
	public Object(int id) {
		...
		List<java.util.Map<String, String>> blockList = parser
				.getChildrenAttributes("Object");
		for (java.util.Map<String, String> data : blockList) {
			Block block = new Block(this);
			block.modifyPos(new Point(Integer.parseInt(data.get("x")), Integer
					.parseInt(data.get("y"))));
			blocks.add(block);
		}
	...
	public void modifyPos(Point pos) {
		...
		if (blocks != null) {
			for (Block block : blocks) {
				block.modifyPos(new Point(pos.getX() + block.getX(), pos.getY()
						+ block.getY()));
			}
		}
	}
